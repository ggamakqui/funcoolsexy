...
public class ProductInsertWithNoDetailImageCommand implements Command {
	...
	public void execute(SqlSession sqlSession, Model model) {
		...
		MultipartFile pThumbnail = mr.getFile("pThumbnail");		// 상세이미지
		...
		String extName1 = originpFilename.substring(originpFilename.lastIndexOf('.') + 1);	// 상세이미지 확장자
		
		// 저장할 파일 이름 만들기 / 업로드 / 파일이름 DB에 저장
		String saveFilename1 = null;	// 상세이미지
		try {
			...
			saveFilename1 = originpFilename.substring(0, originpFilename.lastIndexOf('.')) + "_" + System.currentTimeMillis() + "." + extName1;
			...
			// 2-3) 서버에 저장할 파일을 만든다.
			File saveFile1 = new File(realPath, saveFilename1);	// 상세이미지
			...
			pDAO.insertProductWithNoDetailImage(pName, pPrice, pCategory, pGender, pCompany, saveFilename1, pDescription, pStock1, pStock2, pStock3);
